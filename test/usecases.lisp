...
(subtest "Testing getResult usecase"
  (let ((task1 (make-task
    ...
    (let ((ref1 (register-task task1))
      ...
      (let ((result2-1 (usecase.get-result (task-reference-id ref2))))
        (is-type result2-1
      ...
      (sleep 1)
      (let ((result1 (usecase.get-result (task-reference-id ref1))))
        (is-type result1
                 'usecase.get-result.result)
      ...
      (sleep 2)
      (let ((result2-2 (usecase.get-result (task-reference-id ref2))))
        (is-type result2-2
                 'usecase.get-result.result)
